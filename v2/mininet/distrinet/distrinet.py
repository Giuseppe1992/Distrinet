...
class Distrinet( Mininet ):
    ...
    def __init__( self, topo=None, switch=LxcSwitch, host=LxcNode,
                  ...
                  jump=None, user="root", master=None,
                  **kwargs):
#####        params = {}
    ...
    def OLDstart( self ):
        ...
        for host in self.hosts:
            for intfName in host.nameToIntf.keys():
                ...
                ip = intf.ip
                if ip == None and host.defaultIntf().name == intfName:
                    ip = host.params.get("ip", None)
                intf.config(mac=intf.mac, ip=ip)

            # DSA not really nice, but to make it like mininet
            if "mac" in host.params:
                host.setMAC(mac=host.params["mac"])

        if self.waitConn:
            self.waitConnected(timeout=self.waitConnectionTimeout)
##
    def start( self ):
        "Start controller and switches."
        if not self.built:
...
#                stopped.update( { s: s for s in success } )
    ...
    def _parsePing( pingOutput ):
        ...
        pingOutput = encode(pingOutput)#bytes(str(pingOutput), encoding='utf-8')
        # Check for downed link
        if b'connect: Network is unreachable' in pingOutput:
            return 1, 0
        r = r'(\d+) packets transmitted, (\d+)( packets)? received'
        m = re.search( r, str(pingOutput) )
        if m is None:
            error( '*** XXError: could not parse ping output: %s\n' %
                   pingOutput )
            return 1, 0
